...
func New(opts GRPC11Opts, keyStore bccsp.KeyStore) (bccsp.BCCSP, error) {
	// Init config
	conf := &config{}
	...
	if err != nil {
		return nil, fmt.Errorf("Failed connecting to GRPC11 manager at %s:%s [%s]", opts.Address, opts.Port, err)
	}
	defer conn.Close()
	m := pb.NewGrpc11ManagerClient(conn)

	r, err := m.Load(context.Background(), &pb.LoadInfo{opts.Label, opts.Pin})
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed connecting to GRPC11 dedicated connection at %s [%s]", r.Address, err)
	}
